...
- **三次握手建立连接**
  ...
  首先由客户端向服务端发出申请建立连接的报文，SYN标记位置为1，并初始化序列号`seq=x`告知服务端；服务端收到请求建立连接的报文后，向客户端回复一个同意建立连接并向客户端请求建立建立的报文，ACK和SYN标记位置为1，并设置`ack=x+1`表示确认收到`seq=x`的报文，并把服务端的初始化序列号`seq=y`告知客户端；客户端收到服务端的报文后进入`ESTABLISHED`状态，并回复给服务端一个ACK报文，`ack=y+1`表示确认收到`seq=y`的报文；服务端收到ACK报文后检查正确无误，进入`ESTABLISHED`状态。
